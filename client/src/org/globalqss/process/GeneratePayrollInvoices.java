...
public class GeneratePayrollInvoices  extends SvrProcess {
	...
	protected String doIt() throws Exception
	{
		...
		String whereClause = "HR_Process_ID=? AND HR_Concept_ID IN (SELECT HR_Concept_ID FROM HR_Concept WHERE IsActive='Y' AND C_Charge_ID>0)";
		parameters.add(p_HR_Process_ID);
		if (p_C_BPartner_ID > 0) {
			whereClause += " AND C_BPartner_ID=?";
			parameters.add(p_C_BPartner_ID);
		}
		whereClause += " AND C_InvoiceLine_ID IS NULL";
