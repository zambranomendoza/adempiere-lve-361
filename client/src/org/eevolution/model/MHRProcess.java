...
public class MHRProcess extends X_HR_Process implements DocAction
{
	...
	{
		// JCRA 25/07/2012: Validación para ejecutar la nómina sólo cuando se hayan seleccionado los empleados
		if (!hasEmployees(get_TrxName(), getHR_Process_ID()) && (getDocAction().equals("PR") || getDocAction().equals("CO")))
		...
		if (vp!="")
			...
			vp = attributesWithoutValues(get_TrxName(), getHR_Process_ID());
			if (vp!="")
				throw new AdempiereException(vp);
		}
	...
	*/ 
	...
	/**
	 * Helper Method : return true if the process has employees, else return false
	  * @param trx_name
	  * @param hr_process_id
	 */	
	public static Boolean hasEmployees(String trx_name, int hr_process_id) {
		
		String sConsulta = ""
			+ "SELECT Count(*) "
			+ "FROM   hr_process_employee "
			+ "WHERE  hr_process_id = ? ";
	  	int result = DB.getSQLValue(trx_name ,sConsulta,new Object[] {hr_process_id });
	...
	public static String meetsPrerequisites(String trx_name, int hr_process_id) {
		...
		String sConsulta = ""
				...
				+ "               ON C.c_bpartner_id = E.c_bpartner_id "
				+ "       INNER JOIN hr_process P "
				+ "               ON P.hr_payroll_id = E.hr_payroll_id "
		...
		try{
			...
			while (rSet.next()) 	
				sResult = sResult + rSet.getString(1) + (char)13;
			rSet.close();	
		}
		catch (Exception e) {
			sResult = "Error al ejecutar validación de atributos requeridos: " + e.getMessage();
		}
		
		if (sResult != "") 
			sResult = "Debe ejecutar proceso para asignar los atributos requeridos para los trabajadores " + (char)13 + sResult;
		return sResult;			   
	}
	
	/**
	 * Helper Method : returns the attributes without values 
	  * @param trx_name
	 */	
	public static String attributesWithoutValues(String trx_name, int hr_process_id) {
		String sResult = "";
		PreparedStatement pstmt = null;
		String sConsulta = ""
				+ "SELECT DISTINCT hr_concept.value "
				+ "FROM   hr_attribute "
				+ "       INNER JOIN hr_concept "
				+ "               ON hr_attribute.hr_concept_id = hr_concept.hr_concept_id "
				+ "                  AND hr_concept.isrequiredforworker = 'Y' "
				+ "       INNER JOIN hr_process "
				+ "               ON hr_process.hr_payroll_id = hr_attribute.hr_payroll_id "
				+ "              AND hr_process.hr_process_id = " + hr_process_id + " "
				+ "WHERE  hr_concept.isactive = 'Y' "
				+ "       AND hr_attribute.isactive = 'Y' "
				+ "       AND CASE hr_attribute.columntype "
				+ "             WHEN 'D' THEN servicedate IN ( NULL ) "
				+ "             WHEN 'A' THEN amount IN ( NULL ) "
				+ "             WHEN 'Q' THEN qty IN ( NULL ) "
				+ "             ELSE textmsg IN ( NULL, 'null' ) "
				+ "           END ";
		pstmt= DB.prepareStatement(sConsulta,  trx_name );
		try{
			ResultSet rSet = pstmt.executeQuery();
			...
			sResult = "Error al ejecutar validación de valores en atributos requeridos: " + e.getMessage();
		}
		
		if (sResult != "") 
			sResult = "Debe asignar para todos los trabajadores los respectivos valores en los siguientes atributos requeridos: " + (char)13 + sResult;
		return sResult;			   
	}
}	//	MHRProcess