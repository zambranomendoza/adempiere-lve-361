...
public class VHRActionNotice extends CPanel implements FormPanel,VetoableChangeListener, ActionListener
{
	...
	private void dynInit()
	{
		...
		miniTable.addColumn("Description");			// 9
		//miniTable.addColumn("AD_Rule_ID");		// 10
		//  set details
		...
		//miniTable.setColumnClass(10, String.class, true, Msg.translate(Env.getCtx(), "AD_Rule_ID"));
		//
		miniTable.autoSize();
	}   //  dynInit
	...
	{	
		...
		try {
			...
			while (rs.next()) {
				...
				// miniTable.setValueAt(rs.getString(11), row, 10);		// AD_Rule_ID
				//  prepare next
				row++;
			}
		}
    ...
    //Se agregó la condición hrp.docstatus!='CL' para que solo puedan ser agregadas novedades en nóminas que no han sido cerradas
	/**
	 *  get Process
	...
	private KeyNamePair[] getProcess()
	{
		String sql = MRole.getDefault().addAccessSQL(
				...
				"hrp",MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO) + " AND hrp.IsActive = 'Y' AND hrp.docstatus!='CL' ";
