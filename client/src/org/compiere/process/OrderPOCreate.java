 ...
 *  Contributor: Rafael Salazar C. - rsalazar@dcsla.com - rtsc08@gmail.com from Double Click Sistemas http://www.dcsla.com
 *      Process: Edition -> doIt.  Export only product by category
 * 	<li>
 */
public class OrderPOCreate extends SvrProcess
{
	...
	/** m_product_category			*/
	private int		p_m_product_category ;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			// LVE Localización Venezuela
			else if (name.equals("m_product_category"))
				p_m_product_category = ((BigDecimal)para[i].getParameter()).intValue();
	...
	protected String doIt() throws Exception
	{
		...
		// LVE Localización Venezuela
		//Export only product by category
		if (p_m_product_category!=0){
			sql += " AND NOT EXISTS ( "
				+ " SELECT * FROM C_OrderLine ol "
				+ " INNER JOIN M_Product mp ON ol.m_product_id=mp.m_product_id"
				+ " WHERE o.C_Order_ID=ol.C_Order_ID "
				+ " AND ol.Link_OrderLine_ID IS NOT NULL "
			    + " AND mp.m_product_category_id=  " + p_m_product_category  
			    + " ) ";
		}else{ //export all
			sql += " AND NOT EXISTS (SELECT * FROM C_OrderLine ol WHERE o.C_Order_ID=ol.C_Order_ID AND ol.Link_OrderLine_ID IS NOT NULL ) ";
		}
