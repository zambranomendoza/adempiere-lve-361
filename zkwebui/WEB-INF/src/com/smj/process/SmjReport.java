...
import com.smj.webui.component.SMJReportViewer;

/**
 * Financial Report Engine
 * @version $Id: FinReport.java,v 1.2 2006/07/30 00:51:05 jjanke Exp $
 * @author Jorg Janke
 * @author Armen Rizal, Goodwill Consulting <li>FR [2857076] User Element 1 and
 *         2 completion -
 *         https://sourceforge.net/tracker/?func=detail&aid=2857076
 *         &group_id=176962&atid=879335
 * 
 *  @version <li> SmartJSP: SmjReport.java, 2011/03/01 <ul TYPE ="circle">
 * <li> Esta clase está basada en "FinReport" para la generacion del reporte fianaciero
 * Cambia la forma de generar la ventana de despliegue y realiza el calculo del porcentaje fijo
 * <li> This Class is based in "FinReport" in financial report generation 
 * change Create Report Window and makes the calculation of the fixed percentage </ul>
...
public class SmjReport extends SvrProcess {
	...
	protected void prepare() {
		...
		for (int i = 0; i < para.length; i++) {
			...
			if (para[i].getParameter() == null)
				...
				p_UserElement1_ID = ((BigDecimal) para[i].getParameter())
						.intValue();
			else if (name.equals("UserElement2_ID"))
				p_UserElement2_ID = ((BigDecimal) para[i].getParameter())
						.intValue();
		...
		// Optional Org
		if (p_Org_ID != 0)
			m_parameterWhere.append(" AND ").append(
					MReportTree.getWhereClause(getCtx(), p_PA_Hierarchy_ID,
							MAcctSchemaElement.ELEMENTTYPE_Organization,
							p_Org_ID));
		// Optional BPartner
		if (p_C_BPartner_ID != 0)
			m_parameterWhere.append(" AND ").append(
					MReportTree.getWhereClause(getCtx(), p_PA_Hierarchy_ID,
							MAcctSchemaElement.ELEMENTTYPE_BPartner,
							p_C_BPartner_ID));
		// Optional Product
		if (p_M_Product_ID != 0)
			m_parameterWhere.append(" AND ").append(
		...
		if (p_C_Project_ID != 0)
			m_parameterWhere.append(" AND ").append(
					MReportTree.getWhereClause(getCtx(), p_PA_Hierarchy_ID,
							MAcctSchemaElement.ELEMENTTYPE_Project,
							p_C_Project_ID));
		// Optional Activity
		if (p_C_Activity_ID != 0)
			m_parameterWhere.append(" AND ").append(
					MReportTree.getWhereClause(getCtx(), p_PA_Hierarchy_ID,
							MAcctSchemaElement.ELEMENTTYPE_Activity,
							p_C_Activity_ID));
		// Optional Campaign
		if (p_C_Campaign_ID != 0)
			m_parameterWhere.append(" AND C_Campaign_ID=").append(
					p_C_Campaign_ID);
		...
		if (p_C_SalesRegion_ID != 0)
			m_parameterWhere.append(" AND ").append(
					MReportTree.getWhereClause(getCtx(), p_PA_Hierarchy_ID,
							MAcctSchemaElement.ELEMENTTYPE_SalesRegion,
							p_C_SalesRegion_ID));
		// Optional User1_ID
		if (p_User1_ID != 0)
			m_parameterWhere.append(" AND ").append(
		...
		if (p_User2_ID != 0)
			m_parameterWhere.append(" AND ").append(
					MReportTree.getWhereClause(getCtx(), p_PA_Hierarchy_ID,
							MAcctSchemaElement.ELEMENTTYPE_UserList2,
							p_User2_ID));
		// Optional UserElement1_ID
		if (p_UserElement1_ID != 0)
			m_parameterWhere.append(" AND UserElement1_ID=").append(
					p_UserElement1_ID);
		// Optional UserElement2_ID
		if (p_UserElement2_ID != 0)
			m_parameterWhere.append(" AND UserElement2_ID=").append(
					p_UserElement2_ID);
		...
		//
		setPeriods();
		sb.append(" - C_Period_ID=").append(p_C_Period_ID).append(" - ")
				.append(m_parameterWhere);
		//

		if (p_PA_ReportCube_ID > 0)
			m_parameterWhere.append(" AND PA_ReportCube_ID=").append(
					p_PA_ReportCube_ID);
	...
	private void setPeriods() {
		...
		try {
			...
			while (rs.next()) {
				FinReportPeriod frp = new FinReportPeriod(rs.getInt(1),
	...
	protected String doIt() throws Exception {
		...
		if (p_PA_ReportCube_ID > 0) {
			MReportCube cube = new MReportCube(getCtx(), p_PA_ReportCube_ID,
					get_TrxName());
		...
		int PA_ReportLineSet_ID = m_report.getLineSet()
				.getPA_ReportLineSet_ID();
		StringBuffer sql = new StringBuffer(
				"INSERT INTO T_Report "
						+ "(AD_PInstance_ID, PA_ReportLine_ID, Record_ID,Fact_Acct_ID, SeqNo,LevelNo, Name,Description, smj_HierarchyLevel, smj_ReportLine, smj_fixedPercentage ) "
						+ "SELECT ")
		...
		// Create Report  --> comenta las lineas que configuran el printReport standar de adempiere
		// Create Report  --> coments lines to configure Adempiere standar printReport 
//		if (Ini.isClient())
//			getProcessInfo().setTransientObject(getPrintFormat());
//		else
//			getProcessInfo().setSerializableObject(getPrintFormat());
//		//Crea ventana que muestra el reporte  //personalizacion SMARTJSP
		//Create Report Window  //by SMARTJSP
	...
	private void insertLine(int line) {
		...
		for (int col = 0; col < m_columns.length; col++) {
			...
			if (m_lines[line].getPAAmountType() != null) // line amount type
															// overwrites column
			{
				String sql = m_lines[line].getSelectClause(true);
				select.append(sql);
				info.append(": LineAmtType=").append(
						m_lines[line].getPAAmountType());
			} else if (m_columns[col].getPAAmountType() != null) {
				String sql = m_columns[col].getSelectClause(true);
				select.append(sql);
				info.append(": ColumnAmtType=").append(
						m_columns[col].getPAAmountType());
			} else {
				log.warning("No Amount Type in line: " + m_lines[line]
						+ " or column: " + m_columns[col]);
				continue;
			}
			...
			if (m_lines[line].getPAPeriodType() != null) // line amount type
															// overwrites column
			{
			...
			// Line Where
			String s = m_lines[line].getWhereClause(p_PA_Hierarchy_ID); // (sources,
																		// posting
																		// type)
			...
			if (update.length() > 0)
				update.append(", ");
			update.append("Col_").append(col).append(" = (").append(select)
					.append(")");
			//
		...
		if (update.length() > 0) {
			...
			update.append(" WHERE AD_PInstance_ID=")
					.append(getAD_PInstance_ID())
		...
		for (int line = 0; line < m_lines.length; line++) {
			...
			if (m_lines[line].isCalculationTypeAdd()
				...
				for (int col = 0; col < m_columns.length; col++) {
					if (col > 0)
						sb.append(",");
					sb.append("COALESCE(SUM(r2.Col_").append(col)
							.append("),0)");
				}
				sb.append(" FROM T_Report r2 WHERE r2.AD_PInstance_ID=")
						.append(getAD_PInstance_ID())
						.append(" AND r2.PA_ReportLine_ID IN (");
				if (m_lines[line].isCalculationTypeAdd())
					sb.append(oper_1).append(",").append(oper_2);
				else
					sb.append(getLineIDs(oper_1, oper_2)); // list of columns to
															// add up
				...
				if (no != 1)
					log.log(Level.SEVERE, "(+) #=" + no + " for "
							+ m_lines[line] + " - " + sb.toString());
				else {
			...
			} else // No Add (subtract, percent)
			{
				...
				//fp lleva el sql para actualizar el porcentaja fijo. //smartjsp
				...
				for (int col = 0; col < m_columns.length; col++) {
					...
					// fix bug [ 1563664 ] Errors in values shown in Financial
					// Reports
					// Carlos Ruiz - globalqss
					if (m_lines[line].isCalculationTypeSubtract()) {
						sb.append("-");
						// Solution, for subtraction replace the null with 0,
						// instead of 0.000000001
						sb.append("COALESCE(r2.Col_").append(col).append(",0)");
					} else {
						// Solution, for division don't replace the null,
						// convert 0 to null (avoid ORA-01476)
						sb.append("/");
						sb.append("DECODE (r2.Col_").append(col)
								.append(", 0, NULL, r2.Col_").append(col)
								.append(")");
					}
					// end fix bug [ 1563664 ]
					if (m_lines[line].isCalculationTypePercent()){
						sb.append(" *100");
						//Hace el calculo de porcentaje fijo
						//calculate fixedPercentage
				...
				if (fixPerc){
					...
					if (no != 1)
						log.severe("fixedPercentage #=" + fp.toString());
					else {
						log.fine("fixedPercentage Line=" + line + " - " + m_lines[line]);
						log.finest(fp.toString());
					}
				}//fixPerc
				else{
		...
		for (int col = 0; col < m_columns.length; col++) {
			...
			if (ii_1 < 0) {
				log.log(Level.SEVERE,
						"Column Index for Operator 1 not found - "
								+ m_columns[col]);
				continue;
			}
			// Second Operand
			...
			if (ii_2 < 0) {
				log.log(Level.SEVERE,
						"Column Index for Operator 2 not found - "
								+ m_columns[col]);
				continue;
			}
			...
			if (m_columns[col].isCalculationTypeAdd())
				sb.append("COALESCE(Col_").append(ii_1).append(",0)")
						.append("+").append("COALESCE(Col_").append(ii_2)
						.append(",0)");
			// -
			...
			if (m_columns[col].isCalculationTypePercent()) {
				sb.append("CASE WHEN COALESCE(Col_").append(ii_2)
						...
						.append("/").append("Col_").append(ii_2)
	...
	/**
	 * Regresa el porcentaje Fijo de un valor, dado que sea positivo
	 * Return FixedPercentage just it possitive 
	 * @param trxName
...
//		System.out.println("getFixedPercentage SQL::"+sql.toString());
	...
	private String getLineIDs(int fromID, int toID) {
		...
		for (int line = 0; line < m_lines.length; line++) {
			int PA_ReportLine_ID = m_lines[line].getPA_ReportLine_ID();
			log.finest("Add=" + addToList + " ID=" + PA_ReportLine_ID + " - "
					+ m_lines[line]);
			if (addToList) {
				sb.append(",").append(PA_ReportLine_ID);
				if (PA_ReportLine_ID == lastPA_ReportLine_ID) // done
					break;
			} else if (PA_ReportLine_ID == firstPA_ReportLine_ID) // from
																	// already
																	// added
				addToList = true;
		}
	...
	private FinReportPeriod getPeriod(int relativeOffset) {
		...
		if (m_reportPeriod < 0 || m_reportPeriod >= m_periods.length)
			throw new UnsupportedOperationException(
					"Period index not found - ReportPeriod=" + m_reportPeriod
	...
	private void insertLineDetail() {
		...
		// Clean up empty rows
		StringBuffer sql = new StringBuffer(
				"DELETE FROM T_Report WHERE ABS(LevelNo)<>0")
		...
		sql = new StringBuffer("UPDATE T_Report r SET (Name,Description)=(")
				.append(sql_select)
				.append(") " + "WHERE Fact_Acct_ID <> 0 AND AD_PInstance_ID=")
				.append(getAD_PInstance_ID());
		no = DB.executeUpdate(sql.toString(), get_TrxName());
	...
	/**
	 * Insert Detail Line per Source. For all columns (in a line) with relative
	 * period access - AD_PInstance_ID, PA_ReportLine_ID, variable, 0 - Level 1
	 * 
	 * @param line
	 *            line
	 */
	private void insertLineSource(int line) {
		...
		// Select
		insert.append(") SELECT ").append(getAD_PInstance_ID()).append(",")
		...
		for (int col = 0; col < m_columns.length; col++) {
			...
			// Link
			select.append(" AND fb.").append(variable).append("=x.")
					.append(variable);
			// PostingType
			if (!m_lines[line].isPostingType()) // only if not defined on line
			{
				String PostingType = m_columns[col].getPostingType();
				if (PostingType != null && PostingType.length() > 0)
					select.append(" AND fb.PostingType='").append(PostingType)
							.append("'");
				// globalqss - CarlosRuiz
				if (PostingType.equals(MReportColumn.POSTINGTYPE_Budget)) {
					if (m_columns[col].getGL_Budget_ID() > 0)
			...
			// Parameter Where
			select.append(m_parameterWhere);
			// System.out.println("    c=" + col + ", l=" + line + ": " +
			// select);
			//
		...
		StringBuffer where = new StringBuffer(
				m_lines[line].getWhereClause(p_PA_Hierarchy_ID));
		String s = m_report.getWhereClause();
		if (s != null && s.length() > 0) {
			if (where.length() > 0)
				where.append(" AND ");
		...
		// Set Name,Description
		StringBuffer sql = new StringBuffer(
				"UPDATE T_Report SET (Name,Description)=(")
				.append(m_lines[line].getSourceValueQuery())
				.append("T_Report.Record_ID) "
				//
						+ "WHERE Record_ID <> 0 AND AD_PInstance_ID=")
	...
	/**
	 * Create Trx Line per Source Detail. - AD_PInstance_ID, PA_ReportLine_ID,
	 * variable, Fact_Acct_ID - Level 2
	 * 
	 * @param line
	 *            line
	 * @param variable
	 *            variable, e.g. Account_ID
	 */
	private void insertLineTrx(int line, String variable) {
		...
		for (int col = 0; col < m_columns.length; col++) {
			...
			if (!(m_columns[col].isColumnTypeRelativePeriod() && m_columns[col]
					.getRelativePeriodAsInt() == 0)) {
				insert.append("NULL");
				continue;
			}
			// Amount Type ... Qty
		...
		//
		insert.append(" FROM Fact_Acct WHERE ").append(
				m_lines[line].getWhereClause(p_PA_Hierarchy_ID)); // (sources,
																	// posting
																	// type)
		// Report Where
		String s = m_report.getWhereClause();
	...
	private void deleteUnprintedLines() {
		for (int line = 0; line < m_lines.length; line++) {
			...
			if (!m_lines[line].isPrinted()) {
				String sql = "DELETE FROM T_Report WHERE AD_PInstance_ID="
						+ getAD_PInstance_ID() + " AND PA_ReportLine_ID="
	...
	private void scaleResults() {
		...
		for (int column = 0; column < m_columns.length; column++) {
			...
			if (factor != null) {
				...
				String sql = "UPDATE T_Report SET Col_" + column + "=Col_"
						+ column + "/" + divisor + " WHERE AD_PInstance_ID="
						+ getAD_PInstance_ID();
				int no = DB.executeUpdate(sql, get_TrxName());
				if (no > 0)
