...
CREATE OR REPLACE FUNCTION lve_documentinvoiceporeference(c_invoice_id numeric)
  RETURNS text AS
$BODY$
DECLARE valor text;
BEGIN
	
    SELECT INTO valor (ci.poreference) 
           FROM c_invoice ci
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION lve_documentinvoiceporeference(numeric)
  OWNER TO adempiere;



CREATE OR REPLACE VIEW lve_buy_book AS 
         SELECT ci.ad_org_id, ci.ad_client_id, ci.dateinvoiced AS dateinvoice, cb.taxid AS rif, 
                CASE
                    WHEN ci.isactive = 'Y'::bpchar THEN cb.name
                    ELSE 'ANULADA'::character varying
                END AS name, lri.nroreten AS number_voucher, 
                CASE
                    WHEN cd.docbasetype = 'API'::bpchar THEN ci.poreference
                    ELSE ''::character varying
                END AS nrofac, ci.lve_controlnumber AS numbre_control, 
                CASE
                    WHEN cd.docbasetype = 'APD'::bpchar THEN ci.poreference
                    ELSE ''::character varying
                END AS numbre_debit_note, 
                CASE
                    WHEN cd.docbasetype = 'APC'::bpchar THEN ci.poreference
                    ELSE ''::character varying
                ...
                END AS tipe_transcc, 
                CASE
                    WHEN cd.docbasetype = ANY (ARRAY['APC'::bpchar, 'APD'::bpchar]) THEN lve_documentinvoiceporeference(ci.lve_invoiceaffected_id)
                    ELSE ''::text
                END AS number_invoice_affected, 
                CASE
                    WHEN ci.isactive = 'Y'::bpchar THEN 
                    CASE
                        WHEN cd.docbasetype = 'APC'::bpchar THEN - round(cit.taxbaseamt + cit.taxamt, 2)
                ...
                CASE
                    WHEN ci.isactive = 'Y'::bpchar THEN 
                    CASE
                        WHEN cd.docbasetype = 'APC'::bpchar THEN - round(cit.taxbaseamt, 2)
                ...
                CASE
                    WHEN ci.isactive = 'Y'::bpchar THEN 
                    CASE
                        WHEN cd.docbasetype = 'APC'::bpchar THEN - round(cit.taxamt, 2)
                ...
                CASE
                    WHEN ci.isactive = 'Y'::bpchar THEN 
                    CASE
                        WHEN cd.docbasetype = 'APC'::bpchar THEN - round(lri.retemido, 2)
  ...
  WHERE (ci.grandtotal = lve_calcexento(ci.c_invoice_id) OR ct.rate <> 0::numeric) AND (cd.docbasetype = ANY (ARRAY['APC'::bpchar, 'APD'::bpchar, 'API'::bpchar]))
  GROUP BY ct.rate, ci.ad_org_id, ci.ad_client_id, ci.dateinvoiced, ci.dateacct, cb.taxid, ci.isactive, cb.name, cd.docbasetype, ci.poreference, ci.lve_controlnumber, ci.lve_invoiceaffected_id, cit.taxbaseamt, cit.taxamt, ci.c_invoice_id, lri.nroreten, lri.retemido, lri.dateretention, ci.created
UNION ALL 
         SELECT ci.ad_org_id, ci.ad_client_id, ci.dateinvoiced AS dateinvoice, cb.taxid AS rif, 
                CASE
                    WHEN ci.isactive = 'Y'::bpchar THEN cb.name
...
INSERT INTO adempiere.AD_Process(ad_process_id, ad_client_id, ad_org_id, isactive, created, createdby, updated, updatedby, value, name, description, help, accesslevel, entitytype, procedurename, isreport, isdirectprint, ad_reportview_id, classname, statistic_count, statistic_seconds, ad_printformat_id, workflowvalue, ad_workflow_id, isbetafunctionality, isserverprocess, showhelp, jasperreport, ad_form_id, copyfromprocess)
  VALUES((select max(ad_process_id)+1  from adempiere.AD_Process), 0, 0, 'Y', CURRENT_TIMESTAMP, 100, CURRENT_TIMESTAMP, 100, 'LVE_buy_book', 'Buy Book', 'Sales Book', NULL, '3', 'DCS', NULL, 'Y', 'N', NULL, NULL, 19, 19, NULL, NULL, NULL, 'N', 'N', 'Y', '003LVE_BuyBook.jasper', NULL, NULL)
...
INSERT INTO adempiere.AD_Process_Trl(ad_process_id, ad_language, ad_client_id, ad_org_id, isactive, created, createdby, updated, updatedby, name, description, help, istranslated)
 VALUES((select max(ad_process_id)  from adempiere.AD_Process), 'es_VE', 0, 0, 'Y', CURRENT_TIMESTAMP, 100, CURRENT_TIMESTAMP, 100, 'Libro de Compra', 'Libro de Compra', NULL, 'N')
...
INSERT INTO adempiere.AD_Menu(ad_menu_id, ad_client_id, ad_org_id, isactive, created, createdby, updated, name, updatedby, description, issummary, issotrx, isreadonly, action, ad_window_id, ad_workflow_id, ad_task_id, ad_process_id, ad_form_id, ad_workbench_id, entitytype, iscentrallymaintained)
  VALUES((select max(ad_menu_id)+1  from adempiere.AD_Menu), 0, 0, 'Y',CURRENT_TIMESTAMP, 100, CURRENT_TIMESTAMP, 'Libro de Compras', 100, 'Libro de Compras', 'N', 'N', 'N', 'R', NULL, NULL, NULL, (select max(ad_process_id)  from adempiere.AD_Process), NULL, NULL, 'DCS', 'Y')
