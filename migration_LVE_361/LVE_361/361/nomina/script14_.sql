...
VALUES (@ad_client_id,@AD_Org_ID,(SELECT max(AD_Val_Rule),'hr_period.ad_org_id = @AD_Org_ID@ AND hr_period.hr_payroll_id = @HR_Payroll_ID@',
...
INSERT INTO AD_Val_Rule (AD_Client_ID,AD_Org_ID,AD_Val_Rule_ID,Code,Created,CreatedBy,EntityType,IsActive,Name,Type,Updated,UpdatedBy) 
VALUES (@ad_client_id,@AD_Org_ID,(SELECT max(AD_Val_Rule) FROM AD_Val_Rule),'hr_payroll_id IN (select hr_payroll_id from hr_period where hr_period_id not in 
