...
public class LVE_PayrollVenezuela {
    ...
    public static Timestamp StartDateOfProcessGenerated(String trx_name, int _c_bpartner_id, int ad_org_id, String codContract, boolean isClosed) 
    {
		...
		sConsulta = sConsulta + (isClosed ? "AND pr.docstatus = 'CL' ":  "AND pr.docstatus IN ('IP','CO')");
			date= DB.getSQLValueTS(  trx_name ,sConsulta,new Object[] {c_bpartner_id, ad_org_id, codContract});
    ...
    public static BigDecimal sumConcept (String trx_name, String category, Timestamp _From, Timestamp _To , int _c_bpartner_id, String codContract) {
		...
		String sConsulta = ""
			...
			+ " AND " + category + "='Y' " ;
		if (codContract!=null){
			sConsulta += " AND co.value=? ";
		   	value= DB.getSQLValueBDEx(trx_name ,sConsulta,new Object[] {From,From,To,c_bpartner_id, codContract });
		}else{
			value= DB.getSQLValueBDEx(  trx_name ,sConsulta,new Object[] {From,To,c_bpartner_id });
		}
    ...
    public static BigDecimal sumConceptMonthly (String trx_name, String category, Timestamp _From, Timestamp _To , int _c_bpartner_id, String codContract, int monthDays) {
		...
		value= DB.getSQLValueBDEx(trx_name ,sConsulta,new Object[] {From,To,monthDays,From,From,To,To,c_bpartner_id,codContract});
    ...
    public static BigDecimal sumConcept (String trx_name, Timestamp _From, Timestamp _To, String Concept_Name, int _c_bpartner_id, String codContract) {
		...
		if (codContract!=null){
			sConsulta += " AND co.value=? ";
		   	value= DB.getSQLValueBDEx(trx_name ,sConsulta,new Object[] {From,From,To,Concept_Name,c_bpartner_id, codContract });
		}else{
    ...
    public static Double workDays (String trx_name, Timestamp _From, Timestamp _To, int _hr_payroll_id, String rest, int _c_bpartner_id) {
        ...
        String sQuery = ""
    		+ "  SELECT CASE "
    		+ "  WHEN ct.name = 'Directivo' THEN a.qty "
    		+ "  WHEN ct.name = 'Empleado'  THEN a.qty / 2 "
    		...
    		+ "  AND ct.name in ('Empleado', 'Obrero', 'Directivo')";
    ...
    public static Integer workDays (String trx_name, Timestamp _From, Timestamp _To, int _hr_payroll_id, String rest, String codContract) {
        ...
        String sQuery = ""
    		+ "  SELECT CASE "
    		+ "  WHEN '" + codContract + "' = 'Directivo' THEN a.qty "
    		+ "  WHEN '" + codContract + "' = 'Empleado'  THEN a.qty / 2 "
    		+ "  ELSE Extract(days FROM ( TIMESTAMP '" + To.toString() + "' - TIMESTAMP '" + From.toString() + "' )) + 1 END - "
    		+ "  lve_holidaysinperiod(?, '" + From.toString() + "', '" + To.toString() + "', " + rest + ") - "
    		+ "  lve_daysrest('" + From.toString() + "', '" + To.toString() + "', " + rest + ") "
    		+ "  FROM hr_attribute a INNER JOIN hr_concept c "
	 ...
	 */
	  ...
	  { 	
		...
		String sConsulta = ""
			...
			sConsulta += codContract == null ?  " AND TRIM(co.value) in ('Empleado','Obrero', 'Directivo' ) " : " AND TRIM(co.value) in ( " + codContract + " ) ";
	  ...
	  /**
		 ...
		 */
		  ...
		  { 
			...
			sConsulta += codContract == null ?  " AND co.value in ('Empleado','Obrero', 'Directivo' ) " : " AND co.value in ( " + codContract + " ) ";
		                                      ...
		                                      int _C_BPartner_ID, String CodContract, String isacactive ) throws SQLException
	{
	   BigDecimal resultValue;
		...
		String sConsulta = ""
			...
			+ " AND con.name in (?) "
			+ " AND pr.isactive=? ";
		resultValue= DB.getSQLValueBDEx(  trx_name ,sConsulta,new Object[] {isOption,From, To , _C_BPartner_ID,CodContract ,isacactive});
    ...
    /**
	  ...
	  */
	  ...
	  { 	
		...
		date = DB.getSQLValueTS(trx_name, sQuery, new Object[] {payroll_id, codContract,To,C_BPartner_ID});
	  ...
	  /**
	    ...
	    public static Integer numberDependent (String trx_name, int _C_BPartner_ID, String familyDependent, int minorTo) {
			...
			String sQuery = ""
					+ "SELECT COUNT (*) "
					+ "FROM   LVE_Family fa "
					+ "       INNER JOIN LVE_FamilyDependent fd "
					+ "         ON fd.LVE_familyDependent_id = fa.LVE_familyDependent_id "
	    ...
	    public static  ResultSet QuotasForPaying(String trx_name, Integer ad_org_id, Integer c_bpartner_id, String typeLoan, Integer _Process , Date _paymentDate) throws SQLException
		{
			...
			String sConsulta = ""
					+ "SELECT lq.LVE_loanQuotas_id, "
					+ "       lq.qty, "
					+ "       lq.amount," 
					+ "       le.LVE_loanEmployee_id, "
					+ "       le.isactive "
					+ "FROM   LVE_LoanEmployee le "
					+ "       INNER JOIN LVE_LoanQuotas lq "
					+ "         ON le.LVE_loanEmployee_id = lq.LVE_loanEmployee_id "
					+ "WHERE  le.ad_org_id = ? "
					+ "       AND le.c_bpartner_id = ? "
	    ...
	    public static Double PaymentOfQuota (String trx_name,  Integer ad_org_id, Integer c_bpartner_id, String typeLoan, Date _paymentDate, Integer _Process ) throws SQLException {
	    	...
	    	sConsulta2 = ""
	    		+ "UPDATE LVE_LoanEmployee "
	    		+ "SET    payment = ? "
	    		+ "WHERE  LVE_loanEmployee_id = ? ";
	    	pstmt2= DB.prepareStatement(sConsulta2,  trx_name );
	    	
	    	// Payment Quota
	    	if (rs.getString(5).equals("Y")) { 
	    		sConsulta1 = ""
		    		+ "UPDATE LVE_LoanQuotas "
		    		+ "SET    date1 = ? , "
		    		+ "       payment = ?, "
		    		+ "       hr_process_id = ? "
		    		+ "WHERE  LVE_loanQuotas_id = ? ";
	    ...
	    public static boolean  typeEndEmployee (String trx_name, int org, int _C_BPartner_ID, String contract, String motive) {
			...
			value= DB.getSQLValue(trx_name, sQuery, new Object[] {_C_BPartner_ID, contract, org, motive});
			if (value > 0)
			  return true;
			else
			  return false;
...
//ojo startdate por enddate
	    ...
	    public static Double amountRetention ( int _C_BPartner_ID, MHRProcess process ,String conceptName, Double topSalaryMinimum, 
	    	...
	    	Double salarytToRetain = previousSalary + LVE_PayrollVenezuela.ExecutionOfConcepts(trx_name,process,"isoption1","Y",_Payroll);
