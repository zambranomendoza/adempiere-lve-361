...
public class LVE_GeneratePayrollPayments  extends SvrProcess {
	...
	protected String doIt() throws Exception
	{
		...
		String whereClause = " HR_Concept_ID IN (SELECT c.hr_concept_id FROM LVE_ConceptsPayrollPayments c INNER JOIN LVE_PayrollPayments l ON c.LVE_PayrollPayments_id = l.LVE_PayrollPayments_id WHERE c.IsActive='Y' AND l.LVE_PayrollPayments_id = ? ) ";
		...
		whereClause += " AND C_Payment_ID IS NULL";
		...
		for (MHRMovement movement : movements) {
			...
			if (oldBP != bp.getC_BPartner_ID()) {
				...
				int C_Currency_ID = new Query(getCtx(), MCurrency.Table_Name, "iso_code=?", get_TrxName())
				.setOnlyActiveRecords(true)
				.setParameters("VEB")
				.firstId();					
				...
				payment.setDescription("Empleado: " + C_BPName + " Concepto: " + HR_ConceptName + " Proceso: " + movement.getHR_Process_ID());
